...
def get_company_information():
    unifi_company_information: dict[str, dict[str, ]] = {
        ...
        # example device backing up both OS and network using custom selenium scripts and custom link formats (must have a site and driver keyword argument)
        # this would be used in the case of a UI update with a different URL. You do not necessarily have to change all of these,
        # however more than likely if you have to change one you will have to change the rest. If the URL has something such as a version,
        # you could add an extra field to unifi_backup_settings.py, add it to the format in __post_init__(), and add that in curly braces
        # in the inputted format. I've provided how to do this in comments in these files.
